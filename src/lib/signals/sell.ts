/**
 * Sell Signal Engine
 * Mirror of the scanner buy engine â but for exit decisions.
 *
...
export function computeSellSignal(
  ...
  // ââ 1. Profit target / take profits ââââââââââââââââââ
  ...
  if (unrealizedPct >= 100) {
    ...
    reasons.push({
      ...
      label: 'Doubled â consider taking profits',
    ...
    reasons.push({
      ...
      label: 'Strong profit â partial exit signal',
    ...
    reasons.push({
      ...
      detail: `+${unrealizedPct.toFixed(1)}% â watch for trend reversal`,
    })
  }

  // ââ 2. Price overextended vs averages âââââââââââââââââ
  if (avg7d > 0 && price > 0) {
    ...
    if (vs7d >= 30 && vs30d >= 20) {
      ...
      reasons.push({
        ...
        detail: `+${vs7d.toFixed(1)}% above 7d avg, +${vs30d.toFixed(1)}% above 30d avg â mean reversion likely`,
      })
    } else if (vs7d >= 20) {
      sell_score += 22
      reasons.push({
        ...
        detail: `+${vs7d.toFixed(1)}% above 7d avg â historically corrects`,
      })
    } else if (vs7d >= 12) {
      sell_score += 12
      reasons.push({
  ...
  // ââ 3. Stop loss signals ââââââââââââââââââââââââââââââ
  if (unrealizedPct <= -25 && (item.price_7d_change_pct ?? 0) < -5) {
    sell_score += 28
    reasons.push({
      type: 'stop_loss', weight: 28, direction: 'sell',
      label: 'Stop loss â downtrend accelerating',
    ...
    reasons.push({
      ...
      label: 'Deep loss â reassess thesis',
      detail: `${unrealizedPct.toFixed(1)}% below cost â has the market structure changed?`,
    })
  }

  // ââ 4. Trend reversal (momentum fading) ââââââââââââââ
  const trend7d  = item.price_7d_change_pct  ?? 0
  const trend30d = item.price_30d_change_pct ?? 0

  // Was going up, now reversing â classic exit signal
  if (trend30d >= 15 && trend7d <= -5) {
    sell_score += 20
    reasons.push({
      type: 'trend', weight: 20, direction: 'sell',
      label: 'Trend reversal after rally',
      detail: `30d run +${trend30d.toFixed(1)}% now reversing (7d: ${trend7d.toFixed(1)}%) â momentum fading`,
    })
    ...
    reasons.push({
      ...
      detail: `Strong 30d run (+${trend30d.toFixed(1)}%) stalling â watch for reversal`,
    })
  }

  // ââ 5. Buff buy order floor deteriorating âââââââââââââ
  // If Buff's buy orders drop below current price â market floor is falling
  if (item.buff_buy_order && item.price_usd) {
    ...
    if (buffGap >= 15) {
      ...
      reasons.push({
        ...
        detail: `Current price $${item.price_usd.toFixed(2)} vs Buff buy order $${item.buff_buy_order.toFixed(2)} (${buffGap.toFixed(1)}% gap) â floor is lower`,
      })
    } else if (buffGap >= 8) {
      sell_score += 10
      reasons.push({
  ...
  // ââ 6. Arb: better sell venue exists right now ââââââââ
  if (item.arb_spread_pct && item.arb_spread_pct >= 10 && item.arb_sell_market) {
    ...
    reasons.push({
      ...
      detail: `${item.arb_spread_pct.toFixed(1)}% spread â ${item.arb_sell_market} is paying the most right now`,
    })
  }

  // ââ 7. Upcoming bearish events ââââââââââââââââââââââââ
  ...
  for (const event of relevantEvents.slice(0, 2)) {
    ...
    reasons.push({
      ...
      detail: `${event.title} â historically bearish for ${item.category} items`,
    })
  }

  // ââ 8. Liquidity deteriorating ââââââââââââââââââââââââ
  ...
  if (vol < 5 && totalValue > 500) {
    ...
    reasons.push({
      ...
      detail: `${vol} trades/24h â ${holding.quantity > 1 ? `${holding.quantity} units worth ${(totalValue).toFixed(0)} may take days to exit` : 'very hard to exit at ask'}`,
    })
  }

  // ââ Scanner score deteriorating âââââââââââââââââââââââ
  if (item.score_total != null && item.score_total <= 35 && unrealizedPct > 5) {
    ...
    reasons.push({
      ...
      detail: `Signal score ${item.score_total}/100 â market sees little upside left`,
    })
  }

  // ââ Hold signals (counterweight) âââââââââââââââââââââ
  ...
  if (avg7d > 0 && price < avg7d * 0.95 && unrealizedPct < 10) {
    ...
    reasons.push({
      ...
      detail: 'Price hasn\'t recovered to average â may have more upside',
    })
  }
  if (trend7d >= 5 && unrealizedPct < 20) {
    ...
    reasons.push({
      ...
      detail: `+${trend7d.toFixed(1)}% this week â trend still in your favor`,
    })
  }

  // ââ Clamp and classify ââââââââââââââââââââââââââââââââ
  ...
  let recommendation = ''
  if (urgency === 'none')   recommendation = 'Hold â ino significant exit signals'
  else if (urgency === 'low')    recommendation = `Watch â ${topReasons}`
  else if (urgency === 'medium') recommendation = `Consider selling â ${topReasons}`
  else                           recommendation = `Strong exit signal â  ${topReasons}`
