...
interface PricedItem { assetid: string; market_hash_name: string; icon_url: unknown; tradable: number; marketable: number; price_usd: number | null; item_id: string | null }
...
export function InventoryClient({ steamId, portfolioId }: { steamId: string | null; portfolioId: string | null }) {
  ...
  const { data: items = [], isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch('/api/steam/inventory?steam_id=' + activeId)
      if (!res.ok) { const e = await res.json(); throw new Error(e.error || 'Failed to load inventory') }
      const d = await res.json()
      return (d.items || []) as PricedItem[]
    },
    enabled: !!activeId,
    staleTime: 5 * 60_000,
    retry: 1,
  ...
  const getCond = (name: string) => { const m = name.match(/[(](Factory New|Minimal Wear|Field-Tested|Well-Worn|Battle-Scarred)[)]/); return m ? m[1] : null }
  ...
  const addOne = async (item: PricedItem) => {
    ...
    try {
      const res = await fetch('/api/holdings', { method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  if (!activeId) return (
    <div className='flex flex-col items-center justify-center h-[calc(100vh-88px)] gap-6'>
      <div className='panel p-8 max-w-md w-full'>
        ...
        <p className='font-mono text-[10px] text-muted-4 mt-3'>Find your ID at steamid.io. Inventory must be set to Public.</p>
      </div>
    </div>
  )
  ...
  return (
    <div className='flex flex-col h-[calc(100vh-88px)] overflow-hidden gap-3'>
      <div className='panel px-4 py-3 flex-shrink-0 flex items-center gap-3 flex-wrap'>
        <div><div className='font-mono text-[10px] text-muted-3 uppercase tracking-widest'>Steam Inventory</div><div className='font-mono text-xs text-muted-2'>{activeId}</div></div>
        ...
        <div className='ml-auto flex gap-2'>
          ...
          {portfolioId && filtered.filter(i => i.marketable && !imported.has(i.assetid)).length > 0 && (
            <button onClick={() => filtered.filter(i => i.marketable).forEach(i => addOne(i))} className='btn-primary text-xs py-1.5'>
              Import All ({filtered.filter(i => i.marketable && !imported.has(i.assetid)).length})
            </button>
          )}
        </div>
      </div>
