...
function useInventory(steamId: string | null) {
  return useQuery({
    queryKey: ['steam-inventory', steamId],
    queryFn: async () => {
      if (!steamId) throw new Error('No Steam ID')
      const res = await fetch('/api/steam/inventory?steam_id=' + steamId)
      if (!res.ok) { const e = await res.json(); throw new Error(e.error || 'Failed') }
      const data = await res.json()
      return (data.items || []) as PricedItem[]
    },
    enabled: !!steamId,
    staleTime: 5 * 60_000,
  })
...
export function InventoryClient({ steamId, portfolioId }: { steamId: string | null; portfolioId: string | null }) {
  ...
  const { data: items = [], isLoading, error, refetch } = useInventory(activeId)

  const filtered = items.filter((i: PricedItem) =>
